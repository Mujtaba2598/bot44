...
// Default owner account with password: Mujtabah@2598
...
// ==================== API KEY MANAGEMENT (FIXED VERIFICATION) ====================
app.post('/api/set-api-keys', authenticate, async (req, res) => {
    ...
    try {
        // First test the keys with a simple account endpoint
        ...
        if (!verification.success) {
            ...
            else if (verification.error === 'ip') errorMsg += 'IP not whitelisted. Add your server IP to Binance API restrictions.';
            else if (verification.error === 'format') errorMsg += 'Check key format (no spaces or line breaks).';
...
async function verifyBinanceKeys(apiKey, secretKey) {
    try {
        ...
        if (status === 401 || data?.code === -2015) {
            // Check if it's IP restriction or permission issue
...
// ==================== BINANCE API (simplified for trading) ====================
...
class BinanceAPI {
    ...
    static async syncTime() {
        const endpoints = ['https://api.binance.com/api/v3/time', 'https://api1.binance.com/api/v3/time', 'https://api2.binance.com/api/v3/time'];
        let totalOffset = 0, count = 0;
        for (const endpoint of endpoints) {
            try {
                ...
                totalOffset += (serverTime - localTime);
                count++;
                await this.delay(100);
            } catch (e) {}
        }
        if (count > 0) rateLimit.timeOffset = Math.floor(totalOffset / count);
        rateLimit.lastTimeSync = Date.now();
        return rateLimit.timeOffset;
    }
    
    static async getTimestamp() {
        if (Date.now() - rateLimit.lastTimeSync > 300000 || rateLimit.timeOffset === 0) await this.syncTime();
